...
var metaTag = void 0;
if (metaTag = document.querySelector('meta[name="csrf-token]')) window.axios.defaults.headers.common['X-CSRF-TOKEN'] = metaTag.content;
...
var selects = document.querySelectorAll('select:not(.excludeSelect)');
for (var i = 0; i < selects.length; ++i) {
	if (IsIE) selects[i].className += " browser-default";else new M.Select(selects[i]);
}
var modals = document.querySelectorAll('.modal');
for (var _i = 0; _i < modals.length; ++_i) {
	new M.Modal(modals[_i]);
}tinymce.init({ selector: "#fakeFeedBox", menubar: false });
var charCounters = document.querySelectorAll(".charCount");
for (var _i2 = 0; _i2 < charCounters.length; ++_i2) {
	new M.CharacterCounter(charCounters[_i2]);
}
